...
void testNewOperationPrevious(Relation rel){
    ...
    SortedBagIterator sbi = sb.iterator();
    // case1: prev first position
    sbi.previous();
    assert(!sbi.valid());

    // case2: other positions
    sbi.first();
    for (int i = 0; i < 20; i++) // go until last element
        sbi.next();

    for (int i = 20; i > 0; i--){
        sbi.previous();
        assert(sbi.getCurrent() == i-1);
    }
}
...
void testAllExtended() {
	...
	testNewOperationPrevious(relation2);
}
